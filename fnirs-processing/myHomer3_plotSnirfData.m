function [h,hBGAxis,hChAxis] = myHomer3_plotSnirfDataOnProbe(data, probe, options)
% [h,hBG,hCh] = myHomer3_plotSnirfDataOnProbe(data, probe, options)
%
...
%% Remark
%
% I've been trying ot use Homer 3's plotProbe.m unsuccessfully, in part
% because the documentation is incomplete, and in part becuase it seems
% to use a mixture of legacy of things prior to the snirf format and
% the subsequent to the snirf format.
% This function aims to subtitute that original function.
%
%
%% Input Parameters
%
...
%
% probe -  A Homer3 Snirf ProbeClass object. This contains
...
%
%   .maskProbe - Boolean. With functions such as myHomer_dropChannels
%       some neuroimages (SnirfClass objects) may have been cropped
%       to focus on specific ROIs. But these cropped images will still
...
%       in [mm] for a channel to be considered a short channel. Default
%       is 10.
%   .stim - A list of Homer3 Snirf StimClass objects. The timeline.
%   
%
%% Output Parameters
%
...
%
% 10-May-2023: FOE
%   + Added options to mask the probe and hide channels.
%
...
tmpMeasurementList = myHomer3_unfoldMeasurementList(data);
...
if ~isempty(opt.blockActive)
    ...
    activeMeasurements = ones(nMeasurements,length(opt.blockActive)); %Enlarge to accomodate one column per stimuli
    for iStim = 1:nStim
        ...
        if (mask.sources(iSrc) && mask.detectors(iDet) )
            ...
            if isempty(probe.sourceLabels{iSrc}) || isempty(probe.detectorLabels{iDet})
                ...
                text(channelLocations(iCh,1),...
                     channelLocations(iCh,2),...
                     channelLocations(iCh,3),...
                     sprintf('Ch:%s-%s',probe.sourceLabels{iSrc},...
                                probe.detectorLabels{iDet}), ...
                     'FontSize',opt.fontSize+2,'FontWeight','bold', ...
                     'HorizontalAlignment','center');
            end
...
for iCh = 1:nChannels
    ...
    if isempty(probe.sourceLabels{iSrc}) || isempty(probe.detectorLabels{iDet})
        ...
        title(sprintf('Ch:%s-%s',probe.sourceLabels{iSrc},...
                          probe.detectorLabels{iDet}),...
            'Color',tmpColor);
    end
    box on;
